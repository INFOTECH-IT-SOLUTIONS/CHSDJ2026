...
<h5 class="mb-4 fw-light">
  ...
  <span class="text-muted">Notices ({{ $data->count() }})</span>
              ...
              <tbody>
                @forelse ($data as $notice)
                  <tr>
                    ...
                    <td>
                      @if($notice->file)
                        <a href="{{ asset('public/notice/'.$notice->file) }}" target="_blank" class="text-danger fw-semibold">
                    ...
                    <td>
                      ...
                      {{-- Delete --}}
                      <form method="POST" action="{{ url('panel/admin/notice/delete', $notice->id) }}" class="d-inline">
                        @csrf
                    ...
                    <div class="modal-dialog">
                      <div class="modal-content">
                        <div class="modal-header">
                          <h5 class="modal-title" id="editLabel{{ $notice->id }}">Edit Notice</h5>
                          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <form method="POST" action="{{ url('panel/admin/notice/update', $notice->id) }}" enctype="multipart/form-data">
                          @csrf
                ...
                @empty
                  <tr>
                    <td colspan="6" class="text-center text-muted py-5">No notices found</td>
                  </tr>
                @endforelse
              </tbody>
            </table>
...
<div class="modal fade" id="addNew" tabindex="-1" aria-labelledby="addLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="addLabel"><strong>Add New Notice</strong></h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form method="POST" action="{{ url('panel/admin/notice/add') }}" enctype="multipart/form-data">
        @csrf
        <div class="modal-body">
          <div class="mb-3">
            <label class="form-label">Title</label>
