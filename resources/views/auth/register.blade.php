                    ...
                    <form action="{{ url('register') }}" method="post"
                        ...
                        <div class="mb-2">
                            <a href="{{url('oauth/facebook')}}"
                                class="btn btn-block btn-lg fb-button no-hover text-white"><i
                                    class="fab fa-facebook mr-2"></i> {{trans('misc.sign_up_with')}} Facebook</a>
                        </div>
                        @endif

                        @if($settings->google_login == 'on')
                        <div class="mb-2">
                            <a href="{{url('oauth/google')}}" class="btn btn-block btn-lg google-button no-hover"><img
                                    src="{{url('public/img/google.svg')}}" width="18" height="18"
                                    class="mr-2 align-text-bottom"> {{trans('misc.sign_up_with')}} Google</a>
                        </div>
                        @endif
                            ...
                            <div class="form-group col-md-6">
                                <div class="input-group">
                                    ...
                                    <input type="password" class="form-control" required name="password" id="password"
                                        placeholder="{{ trans('auth.password') }}" title="{{ trans('auth.password') }}"
                                        autocomplete="off">
                                </div>
                            </div>
                            ...
                            <div class="form-group col-md-6">
                                <div class="input-group">
                                    ...
                                    <input type="password" class="form-control" required name="password_confirmation"
                                        ...
                                        title="{{ trans('auth.confirm_password') }}" autocomplete="off">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-check my-1">
                            <input type="checkbox" required @if( old('agree_gdpr') ) checked="checked" @endif
                                class="form-check-input" name="agree_gdpr" id="customControlInline" value="1">
                            <label class="form-check-label" for="customControlInline">{{ trans('admin.i_agree_gdpr') }}
                                @if($settings->link_privacy != '')
                                <a href="{{$settings->link_privacy}}" target="_blank">{{ trans('admin.privacy_policy')
                                    }}</a>
                                @endif
                            </label>
                        ...
                        {!! NoCaptcha::renderJs() !!}
                        @else
                        <button type="submit" class="btn btn-block mt-2 py-2 btn-primary font-weight-light">{{
                            trans('auth.sign_up') }}</button>
                        @endif

                        @if ($settings->captcha == 'on')
                        <small class="btn-block text-center">{{trans('misc.protected_recaptcha')}} <a
                                href="https://policies.google.com/privacy" target="_blank">{{trans('misc.privacy')}}</a>
                            - <a href="https://policies.google.com/terms"
                                target="_blank">{{trans('misc.terms')}}</a></small>
                        @endif
